...
message.channel.send(new Discord.MessageEmbed()//TRjudl
...
message.guild.roles.create({ data: { name: '🤯・Dost' }, reason: 'ayn' }).then(s => s.setColor('#ffff00'))
...
message.guild.channels.create('🎮・APEX', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(10));
    ...
    message.guild.channels.create('●▬▬▬▬๑「🏆」๑▬▬▬▬●', {type: 'category'}).then(parent => {
message.guild.channels.create('TRjudl BOT #1', {type: 'text'}).then(c => c.setParent(parent.id));
});
});
